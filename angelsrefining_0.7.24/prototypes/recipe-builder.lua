...
local function p_split_key(t, k, e)
...
local function p_result_merge(target, base, patch, split, e)
   ...
   local p, pc, ps
   if split then
      b, bc, bs = p_split_key(base, "result", e), p_split_key(base, "result_count", e), p_split_key(base, "results", e)
   ...
   if ps then
      target.results = p_merge_item_lists(bs or {{ name = b, type = "item", amount = bc or 1 }}, ps)
   elseif p then
      target.result = p
      target.result_count = p_merge(bc, pc, "result_count")
   elseif bs then
      target.results = bs
   else
      target.result = b
