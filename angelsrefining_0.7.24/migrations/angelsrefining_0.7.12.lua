  ...
  force.reset_technologies()
	if game.item_prototypes["pellet-aluminium"] then
		...
		if force.technologies["water-washing-2"].researched then
			...
			force.recipes["solid-nodule"].enabled = true
		end
		if force.technologies["nodule-processing"].researched then
			force.recipes["nodule-crushed"].enabled = true
			force.recipes["nodule-crushed-liquifying"].enabled = true
			force.recipes["nodule-crystallization-1"].enabled = true
			force.recipes["nodule-crystallization-2"].enabled = true
		end
	end
end