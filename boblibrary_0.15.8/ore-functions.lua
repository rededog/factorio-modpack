...
-- note: Use size 1 for stone size, 1.5 for iron/copper/coal size, over 2 is untested.
  ...
  autoplace.richness_multiplier_distance_bonus = 30
  autoplace.coverage = size / 75

  autoplace.peaks = {}
  if inputs.starting_area == 1 then
    autoplace.starting_area_size = 600 * size / 75
    autoplace.starting_area_amount = 1000 * size
  end
...
function bobmods.lib.resource.create(inputs)
  ...
  {
    {
      ...
      stages = sheet,
      stage_counts = inputs.stage_counts or bobmods.lib.resource.stage_counts(sheet.sheet.variation_count, inputs.stage_mult or 1),
  ...
  if inputs.required_fluid then
    data.raw.resource[inputs.name].required_fluid = inputs.required_fluid
    data.raw.resource[inputs.name].fluid_amount = inputs.fluid_amount or 10
  end
...
function bobmods.lib.resource.create_icon(inputs)
  local icon = { icon = "__boblibrary__/graphics/icons/ore-1.png", tint = inputs.tint }
  if inputs.sprite then
...
function bobmods.lib.resource.generate_data_stage(inputs)
  if inputs.name then
    ...
    if inputs.item and inputs.item.create then
      ...
      local item = {name = inputs.name, icon = inputs.icon, subgroup = inputs.item.subgroup, stack_size = inputs.item.stack_size, tint = inputs.tint}
      if inputs.sprite then
