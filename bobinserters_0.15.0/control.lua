...
offset: the index in table bobmods.inserters.offset_positions to define the offset location within the drop tile.
  ...
  draw_gui = function(data)
    ...
    if near_unlocked then
      data.gui.add({type = "table", name = "main_table", colspan = 5, style = "bob-table"})
    else
      data.gui.add({type = "table", name = "main_table", colspan = 3, style = "bob-table"})
    end

    data.gui.main_table.add({type = "table", name = "pickup", colspan = 1, style = "bob-table"})
    data.gui.main_table.add({type = "label", name = "spacer1", caption = "     " })
    data.gui.main_table.add({type = "table", name = "drop", colspan = 1, style = "bob-table"})
    ...
    local gui_pickup = data.gui.main_table.pickup.add({type = "table", name = data.name .. "_pickup", colspan = colspan, style = "bob-table"})
    local gui_drop = data.gui.main_table.drop.add({type = "table", name = data.name .. "_drop", colspan = colspan, style = "bob-table"})

    bobmods.inserters.draw_button_grid(gui_pickup, pickup_position, "bob-inserter-checkbox-pickup", long_unlocked, more_unlocked, islong, long2_unlocked, more2_unlocked, islong2, isshort)
    bobmods.inserters.draw_button_grid(gui_drop, drop_position, "bob-inserter-checkbox-drop", long_unlocked, more_unlocked, islong, long2_unlocked, more2_unlocked, islong2, isshort)

    if near_unlocked then
      data.gui.main_table.add({type = "label", name = "spacer2", caption = "     " })
      data.gui.main_table.add({type = "table", name = "offset", colspan = 1, style = "bob-table"})
      data.gui.main_table.offset.add({type = "label", name = "entity_name", caption = {"gui.bob-inserter-offset"}})
      local gui_offset = data.gui.main_table.offset.add({type = "table", name = data.name .. "_offset", colspan = 3, style = "bob-table"})
